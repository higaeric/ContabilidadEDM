...
class DataGridViewPrinter
{
    ...
    private DataGridView TheDataGridView2;
    ...
    private float coordXGrid;
    private float anchos;
    ...
    {
        aTitleColor = Construct(aDataGridView, aPrintDocument, CenterOnPage, WithTitle, aTitleText, aTitleFont, aTitleColor, WithPaging, null);
    }
    ...
    bool CenterOnPage, bool WithTitle,
    string aTitleText, Font aTitleFont,
    Color aTitleColor, bool WithPaging, DataGridView footer)
    {
        Construct(aDataGridView, aPrintDocument, CenterOnPage, WithTitle,
            aTitleText, aTitleFont, aTitleColor, WithPaging, footer);
    }

    private Color Construct(DataGridView aDataGridView, PrintDocument aPrintDocument, bool CenterOnPage, 
        bool WithTitle, string aTitleText, Font aTitleFont, Color aTitleColor, bool WithPaging,
        DataGridView footer)
    {
        TheDataGridView2 = footer;
    ...
    private void Calculate2(Graphics g)
    {
        if (PageNumber == 0)
        {
            SizeF tmpSize = new SizeF();
            ...
            TheDataGridViewWidth = 0;
            for (int i = 0; i < TheDataGridView2.Columns.Count; i++)
            {
                tmpFont = TheDataGridView2.ColumnHeadersDefaultCellStyle.Font;
                if (tmpFont == null)
                    tmpFont = TheDataGridView2.DefaultCellStyle.Font;

                tmpSize = g.MeasureString(
                          TheDataGridView2.Columns[i].HeaderText,
                          tmpFont);
                tmpWidth = tmpSize.Width;
                RowHeaderHeight = tmpSize.Height;

                for (int j = 0; j < TheDataGridView2.Rows.Count; j++)
                {
                    tmpFont = TheDataGridView2.Rows[j].DefaultCellStyle.Font;
                    if (tmpFont == null)
                        tmpFont = TheDataGridView2.DefaultCellStyle.Font;
                    ...
                    tmpSize =
                        g.MeasureString(
                        TheDataGridView2.Rows[j].Cells[i].
                                 EditedFormattedValue.ToString(),
                        tmpFont);
                    if (tmpSize.Width > tmpWidth)
                        tmpWidth = tmpSize.Width;
                }
                if (TheDataGridView2.Columns[i].Visible)
                    TheDataGridViewWidth += tmpWidth;
                ColumnsWidth.Add(tmpWidth);
            }
            int k;
            ...
            int mStartPoint = 0;
            for (k = 0; k < TheDataGridView2.Columns.Count; k++)
                if (TheDataGridView2.Columns[k].Visible)
                {
                    mStartPoint = k;
                    break;
                }

            int mEndPoint = TheDataGridView2.Columns.Count;
            for (k = TheDataGridView2.Columns.Count - 1; k >= 0; k--)
                if (TheDataGridView2.Columns[k].Visible)
                {
                    mEndPoint = k + 1;
                    break;
    ...
    private void DrawHeader(Graphics g)
    {
        ...
        #region "Printing the page number (if isWithPaging is set to true)"
        if (IsWithPaging)
        {
            PageNumber++;
        ...
        #endregion
        #region "Printing the title (if IsWithTitle is set to true)"
        if (IsWithTitle)
        {
        ...
        anchos = mColumnPointsWidth[mColumnPoint];
        ...
        #region "Printing each visible cell of the header row"
    ...
    private bool DrawRows(Graphics g)
    {
        ...
        #region "Printing each visible cell"
    ...
    private void DrawFooterTop(Graphics g)
    {
        CurrentY += RowHeaderHeight;

        // Calculating the starting x coordinate
        // that the printing process will start from
        float CurrentX = coordXGrid;

        // Setting the HeaderFore style
        Color HeaderForeColor =
              TheDataGridView2.ColumnHeadersDefaultCellStyle.ForeColor;
        if (HeaderForeColor.IsEmpty)
            // If there is no special HeaderFore style,
            // then use the default DataGridView style
            HeaderForeColor = TheDataGridView2.DefaultCellStyle.ForeColor;
        SolidBrush HeaderForeBrush = new SolidBrush(HeaderForeColor);

        // Setting the HeaderBack style
        Color HeaderBackColor =
              TheDataGridView2.ColumnHeadersDefaultCellStyle.BackColor;
        if (HeaderBackColor.IsEmpty)
            // If there is no special HeaderBack style,
            // then use the default DataGridView style
            HeaderBackColor = TheDataGridView2.DefaultCellStyle.BackColor;
        SolidBrush HeaderBackBrush = new SolidBrush(HeaderBackColor);
        ...
        // (derived from the GridColor property
        // of the DataGridView control)
        Pen TheLinePen = new Pen(TheDataGridView2.GridColor, 1);

        // Setting the HeaderFont style
        Font HeaderFont = TheDataGridView2.ColumnHeadersDefaultCellStyle.Font;
        if (HeaderFont == null)
            // If there is no special HeaderFont style,
            // then use the default DataGridView font style
            HeaderFont = TheDataGridView2.DefaultCellStyle.Font;

        // Calculating and drawing the HeaderBounds        
        RectangleF HeaderBounds = new RectangleF(CurrentX, CurrentY,
            anchos, RowHeaderHeight);
        g.FillRectangle(HeaderBackBrush, HeaderBounds);
        ...
        {
            // If the column is not visible then ignore this iteration
            if (!TheDataGridView2.Columns[i].Visible) continue;

            ColumnWidth = ColumnsWidth[i];

            // Check the CurrentCell alignment
            // and apply it to the CellFormat
            if (TheDataGridView2.ColumnHeadersDefaultCellStyle.
                       Alignment.ToString().Contains("Right"))
                CellFormat.Alignment = StringAlignment.Far;
            else if (TheDataGridView2.ColumnHeadersDefaultCellStyle.
                     Alignment.ToString().Contains("Center"))
                CellFormat.Alignment = StringAlignment.Center;
            else
                CellFormat.Alignment = StringAlignment.Near;
            ...
            // Printing the cell text
            g.DrawString(TheDataGridView2.Columns[i].HeaderText,
                         HeaderFont, HeaderForeBrush,
               CellBounds, CellFormat);

            // Drawing the cell bounds
            // Draw the cell border only if the HeaderBorderStyle is not None
            if (TheDataGridView2.RowHeadersBorderStyle !=
                            DataGridViewHeaderBorderStyle.None)
                g.DrawRectangle(TheLinePen, CurrentX, CurrentY, ColumnWidth,
                    RowHeaderHeight);
    ...
    private bool DrawFooter(Graphics g)
    {
        float CurrentX = coordXGrid;
        ...
        float ColumnWidth;
        #region"Contenido"
        while (CurrentRow < TheDataGridView2.Rows.Count)
        {
            // Print the cells of the CurrentRow only if that row is visible
            if (TheDataGridView2.Rows[CurrentRow].Visible)
            {
                // Setting the row font style
                RowFont = TheDataGridView2.Rows[CurrentRow].DefaultCellStyle.Font;
                // If the there is no special font style of the CurrentRow,
                // then use the default one associated with the DataGridView control
                if (RowFont == null)
                    RowFont = TheDataGridView2.DefaultCellStyle.Font;

                // Setting the RowFore style
                RowForeColor =
                  TheDataGridView2.Rows[CurrentRow].DefaultCellStyle.ForeColor;
                // If the there is no special RowFore style of the CurrentRow,
                // then use the default one associated with the DataGridView control
                if (RowForeColor.IsEmpty)
                    RowForeColor = TheDataGridView2.DefaultCellStyle.ForeColor;
                RowForeBrush = new SolidBrush(RowForeColor);

                // Setting the RowBack (for even rows) and the RowAlternatingBack
                // (for odd rows) styles
                RowBackColor =
                  TheDataGridView2.Rows[CurrentRow].DefaultCellStyle.BackColor;
                // If the there is no special RowBack style of the CurrentRow,
                // then use the default one associated with the DataGridView control
                if (RowBackColor.IsEmpty)
                {
                    RowBackBrush = new SolidBrush(
                          TheDataGridView2.DefaultCellStyle.BackColor);
                    RowAlternatingBackBrush = new
                        SolidBrush(
                        TheDataGridView2.AlternatingRowsDefaultCellStyle.BackColor);
                }
                // If the there is a special RowBack style of the CurrentRow,
                // then use it for both the RowBack and the RowAlternatingBack styles
                else
                    ...
                    CurrentCell++)
                {
                    // If the cell is belong to invisible
                    // column, then ignore this iteration
                    if (!TheDataGridView2.Columns[CurrentCell].Visible) continue;

                    // Check the CurrentCell alignment
                    // and apply it to the CellFormat
                    if (TheDataGridView2.Columns[CurrentCell].DefaultCellStyle.
                            Alignment.ToString().Contains("Right"))
                        CellFormat.Alignment = StringAlignment.Far;
                    else if (TheDataGridView2.Columns[CurrentCell].DefaultCellStyle.
                            Alignment.ToString().Contains("Center"))
                        CellFormat.Alignment = StringAlignment.Center;
                    else
                        CellFormat.Alignment = StringAlignment.Near;
                    ...
                    // Printing the cell text
                    g.DrawString(
                      TheDataGridView2.Rows[CurrentRow].Cells[CurrentCell].
                      EditedFormattedValue.ToString(), RowFont, RowForeBrush,
                      CellBounds, CellFormat);
                    ...
                    // Draw the cell border only
                    // if the CellBorderStyle is not None
                    if (TheDataGridView2.CellBorderStyle !=
                                DataGridViewCellBorderStyle.None)
                        g.DrawRectangle(TheLinePen, CurrentX, CurrentY,
                              ColumnWidth, RowsHeight[CurrentRow]);
